...
export const LibraryBookCard = memo(function LibraryBookCard({
  ...
  const tilt = index % 3 === 0 ? "-rotate-[0.35deg]" : index % 3 === 1 ? "rotate-[0.3deg]" : "-rotate-[0.15deg]";
  ...
  return (
    <motion.div
      ...
      onClick={onOpen}
      className={cn(
        "group flex cursor-pointer gap-4 border-2 border-neutral-900 bg-white p-4 shadow-[4px_4px_0_0_rgb(0_0_0_/_0.1)] transition-all hover:z-10 hover:-translate-y-0.5 hover:shadow-[6px_6px_0_0_rgb(0_0_0_/_0.12)] motion-reduce:rotate-0",
        tilt,
        className,
      )}
    >
      <div className="relative h-[7.25rem] w-[4.75rem] shrink-0 overflow-hidden rounded-sm border-2 border-neutral-900 bg-gradient-to-br from-teal-50 to-neutral-100">
        {coverUrl ? (
          <img src={coverUrl} alt="" loading="lazy" className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full w-full flex-col items-center justify-center gap-1 p-2 text-center">
            <BookOpen className="h-6 w-6 text-teal-800/60" aria-hidden />
            <span className="text-[9px] font-semibold uppercase tracking-wider text-neutral-500">Cover</span>
          </div>
        )}
      </div>
      <div className="min-w-0 flex-1">
        <div className="mb-2 flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h3 className="text-lg font-bold leading-tight text-foreground group-hover:underline">{novel.title}</h3>
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <span className="hidden rounded-sm border border-border bg-primary/15 px-2 py-0.5 text-xs font-mono font-semibold text-foreground sm:inline">
              {totalWords.toLocaleString()} w
            </span>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
          <span className="font-medium text-neutral-800">{novel.genre ?? "—"}</span>
          ...
          {novel.seriesPosition != null && (
            <>
              ...
              <span className="rounded bg-teal-600/15 px-1.5 py-0.5 font-medium text-teal-900">
